...
This document contains two sets of functions in the child chunk *captions_html_latex.rmd*. One set is used for html documents and the other for pdf documents. For pdf documents it does not work with the output *pdf_document* : it is necessary to use *bookdown::pdf_document2* . With the default Pandoc template a table of contents is always produced for this output type. In a later version we will correct this by adapting and using (a copy of) this template. 
